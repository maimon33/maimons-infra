...
VERSION = os.getenv("MONITORING_VERSION", "dev")
...
class HistoryStore:
    ...
    def overview_24h(self) -> dict[str, Any]:
        """Get high/low values for the last 24 hours only."""
        return self.overview(days=1)
    ...
    def refresh(self) -> None:
        ...
        try:
            try:
                ...
                if error.code == 401:
                    detail = "Invalid API token. Verify CLOUDFLARE_API_TOKEN is set and valid."
                raise RuntimeError(
...
<style>
:root{color-scheme:dark;--bg:#0b1017;--panel:#121a24;--line:#223044;--text:#e9f0f7;--muted:#8998aa;--green:#45d483;--red:#ff6978;--blue:#58a6ff;--orange:#ffad45}*{box-sizing:border-box}body{margin:0;background:radial-gradient(circle at 15% 0,#132238 0,transparent 30%),var(--bg);color:var(--text);font:14px ui-sans-serif,system-ui,-apple-system,sans-serif}main{max-width:1200px;margin:auto;padding:32px 20px 60px}header{display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:14px;gap:20px;flex-wrap:wrap}h1{font-size:25px;margin:0 0 5px;letter-spacing:-.02em}h2{font-size:15px;margin:0 0 14px}.muted{color:var(--muted)}.controls{display:flex;gap:12px;align-items:center;flex-wrap:wrap}select{background:rgba(18,26,36,.92);border:1px solid var(--line);border-radius:6px;color:var(--text);padding:6px 10px;cursor:pointer}button{background:rgba(18,26,36,.92);border:1px solid var(--line);border-radius:6px;color:var(--text);padding:6px 10px;cursor:pointer;transition:all .2s}.page-nav{display:flex;gap:8px}button.active{border-color:var(--blue);color:var(--blue)}.loading{display:flex;align-items:center;gap:10px;border:1px solid #294260;background:#101c2b;color:#b8d7ff;border-radius:9px;padding:11px 13px;margin-bottom:18px;transition:opacity .2s}.loading.hidden{display:none}.spinner{width:14px;height:14px;border:2px solid #35516f;border-top-color:var(--blue);border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px}.card,.section{background:rgba(18,26,36,.92);border:1px solid var(--line);border-radius:12px}.card{padding:16px}.card.bad{border-color:#692b37;background:rgba(53,20,28,.94)}.card.bad .value{color:var(--red)}.label{color:var(--muted);font-size:12px}.value{font-size:27px;font-weight:650;margin-top:8px}.section{padding:18px;margin-bottom:16px;overflow:auto}.row{display:flex;gap:8px;align-items:center}.dot{width:8px;height:8px;border-radius:50%;background:var(--green)}.dot.bad{background:var(--red)}table{border-collapse:collapse;width:100%;min-width:720px}th,td{text-align:left;padding:11px 8px;border-top:1px solid var(--line)}th{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.06em}code{font-family:ui-monospace,SFMono-Regular,monospace;font-size:12px;color:#b9c7d7}.bar{height:5px;background:#233044;border-radius:4px;overflow:hidden;margin-top:5px;width:110px}.bar i{display:block;height:100%;background:var(--blue);border-radius:4px}.error{color:var(--red)}.pill{padding:3px 7px;border-radius:20px;background:#153421;color:var(--green);font-size:11px}.pill.bad{background:#3a1a21;color:var(--red)}.page{display:none}.page.active{display:block}@media(max-width:760px){.grid{grid-template-columns:repeat(2,1fr)}header{align-items:flex-start;flex-direction:column;gap:8px}}</style>
</head><body><main><header><div><h1>Maimons Monitor <span style="font-size:12px;color:var(--muted)">v''' + VERSION + r'''</span></h1><div class="muted">Live Docker runtime and Cloudflare edge metrics.</div></div><div><div class="controls"><select id="pollingInterval"><option value="0">Manual refresh</option><option value="60">1 minute</option><option value="300" selected>5 minutes</option><option value="900">15 minutes</option></select><button id="refreshBtn">⟳ Refresh</button><div class="page-nav"><button id="dashboardBtn" class="active">Dashboard</button><button id="historyBtn">History</button><button id="logsBtn">Logs</button></div></div><div id="stamp" class="muted" aria-live="polite">Connecting…</div></div></header>
<div id="dashboard" class="page active"><div class="loading" id="loading" role="status" aria-live="polite"><span class="spinner" aria-hidden="true"></span><span id="loading-text">Connecting to Docker, Tunnel, history, and Cloudflare…</span></div><div class="grid" id="cards"></div><section class="section"><h2>Containers</h2><div id="containers"><span class="muted">Waiting for the first live sample…</span></div></section><section class="section"><h2>Metric highs & lows — last 24 hours</h2><div id="history24h"><span class="muted">Collecting metrics…</span></div></section><section class="section"><h2>Cloudflare edge — trailing 24 hours</h2><div id="edge"><span class="muted">Loading edge analytics…</span></div></section></div>
<div id="historyPage" class="page"><div id="historyContent"><span class="muted">Loading detailed history…</span></div></div>
<div id="logsPage" class="page"><div id="logsContent"><span class="muted">Select a container to view logs.</span></div><div id="logsData"><span class="muted">No logs selected.</span></div></div></main>
<script>
...
let currentInterval=300000,refreshTimer=null;
...
function edgeTable(d){if(!d.configured)return '<div class="muted">Add Cloudflare read-only API token and zone ID to enable edge analytics.</div>';if(!d.ok)return `<div class="error">${esc(d.error)}</div>`;if(!d.hostnames.length)return '<div class="muted">No matching hostname traffic in the last 24 hours.</div>';return `<table><thead><tr><th>Hostname</th><th>Requests</th><th>Visits</th><th>Edge volume</th></tr></thead><tbody>${d.hostnames.map(x=>`<tr><td><strong>${esc(x.hostname)}</strong></td><td>${fmt(x.requests)}</td><td>${fmt(x.visits)}</td><td>${bytes(x.bytes)}</td></tr>`).join('')}</tbody></table>`}
async function refresh(){try{const [statusResponse,historyResponse]=await Promise.all([fetch('/api/status',{cache:'no-store'}),fetch('/api/history/overview?hours=24',{cache:'no-store'})]);if(!statusResponse.ok||!historyResponse.ok)throw new Error(`Live update failed (${statusResponse.status}/${historyResponse.status})`);const d=await statusResponse.json(),h=await historyResponse.json();document.querySelector('#cards').innerHTML=cards(d);document.querySelector('#containers').innerHTML=containerTable(d.docker);document.querySelector('#history24h').innerHTML=historyTable(h);document.querySelector('#edge').innerHTML=edgeTable(d.cloudflare);document.querySelector('#loading').classList.add('hidden');document.querySelector('#stamp').textContent=`Live · updated ${new Date(d.generatedAt).toLocaleTimeString()} · ${d.viewer||'Cloudflare Access'}`;}catch(e){document.querySelector('#loading-text').textContent=`Still waiting for live metrics: ${e}`;document.querySelector('#stamp').innerHTML='<span class="error">Live update unavailable</span>'}}
function setInterval_(ms){currentInterval=ms;clearInterval(refreshTimer);if(ms>0)refreshTimer=setInterval(refresh,ms)}
function showPage(page){document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));document.querySelectorAll('button').forEach(b=>b.classList.remove('active'));document.querySelector('#'+page).classList.add('active');document.querySelector('#'+page.replace('Page','Btn')).classList.add('active')}
async function loadHistory(){const h=await fetch('/api/history/overview',{cache:'no-store'}).then(r=>r.json());if(!h.ok)return `<div class="error">${esc(h.error)}</div>`;if(!h.metrics.length)return '<div class="muted">No history data yet. History is retained for '+h.retentionDays+' days.</div>';return `<section class="section"><h2>Historical Highs & Lows (${h.retentionDays} day retention)</h2><table><thead><tr><th>Container</th><th>Metric</th><th>Low</th><th>High</th><th>Period</th></tr></thead><tbody>${h.metrics.map(x=>`<tr><td><strong>${esc(x.container)}</strong></td><td><code>${esc(x.metric)}</code></td><td>${metricValue(x.metric,x.low)}</td><td>${metricValue(x.metric,x.high)}</td><td>${new Date(x.firstBucket).toLocaleDateString()}–${new Date(x.lastBucket).toLocaleDateString()}</td></tr>`).join('')}</tbody></table></section>`}
async function loadLogs(){const d=await fetch('/api/status',{cache:'no-store'}).then(r=>r.json());const containers=(d.docker?.containers||[]).filter(c=>c.state==='running');return `<section class="section"><h2>Container Logs</h2><div>${containers.length?containers.map(c=>`<button onclick="viewLog('${esc(c.name)}')">${esc(c.name)}</button>`).join(''):'<span class="muted">No running containers.</span>'}</div></section>`}
async function viewLog(name){try{const r=await fetch(`/api/logs?service=${encodeURIComponent(name)}&tail=100`,{cache:'no-store'});const d=await r.json();document.querySelector('#logsData').innerHTML=`<section class="section"><h2>Logs: ${esc(name)}</h2><pre>${esc(d.logs||'No logs')}</pre></section>`}catch(e){document.querySelector('#logsData').innerHTML=`<div class="error">Error loading logs: ${e}</div>`}}
refresh();setInterval_(currentInterval);document.querySelector('#pollingInterval').addEventListener('change',(e)=>{const val=parseInt(e.target.value);setInterval_(val*1000)});document.querySelector('#refreshBtn').addEventListener('click',refresh);document.querySelector('#dashboardBtn').addEventListener('click',()=>showPage('dashboard'));document.querySelector('#historyBtn').addEventListener('click',async()=>{showPage('historyPage');document.querySelector('#historyContent').innerHTML=await loadHistory()});document.querySelector('#logsBtn').addEventListener('click',async()=>{showPage('logsPage');document.querySelector('#logsContent').innerHTML=await loadLogs()});
</script></body></html>'''
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if request_url.path == "/":
            ...
            try:
                ...
                days = max(1, min(hours // 24, HISTORY_RETENTION_DAYS))
            except ValueError:
                days = HISTORY_RETENTION_DAYS
            self.send_body(
                200,
                json.dumps(history.overview(days=days), separators=(",", ":")).encode(),
                "application/json",
            )
